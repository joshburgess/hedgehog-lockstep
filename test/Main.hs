...
import Test.UnitCoverage (prop_applyOp, prop_gvarLabel, prop_mkGVarId)
...
main = do
  ...
  ok9 <- check prop_buggyCounterDetected
  let bugDetected = not ok9
  putStrLn $ if bugDetected
    then "  Buggy model correctly detected"
    else "  Buggy model was NOT detected (bug in lockstep!)"
  if and [ok1, ok2, ok3, ok4, ok5, ok6, ok7, ok8, bugDetected]
    then exitSuccess
    else exitFailure
